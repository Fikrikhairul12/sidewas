...
use App\Models\EksternalTindakLanjut;
...
class TindakLanjutEksternalController extends Controller
{
    public function index(Request $request)
    {
        $user = User::find(Auth::id());

        if (!$user || !$user->canAccessEksternalTindakLanjut()) {
            abort(403, 'Anda tidak memiliki akses ke halaman tindak lanjut EKSTERNAL.');
        }

        $butirsRiwayatQuery = EksternalButir::with([
            'record.creator',
            'cluster',
            'subCluster',
        ...
        if (!$user->isSuperAdmin() && !$user->hasRoleType('admin_eksternal') && !$user->hasRoleType('moderator_eksternal')) {
        ...
        if ($request->filled('keyword')) {
            ...
            $rows = $rows->filter(function ($row) use ($keyword) {
                ...
                $values = [
                    ...
                    $butir?->tanggal_eksternal?->format('d/m/Y'),
        ...
        $butirSiapTindakLanjutQuery = EksternalButir::with([
            'record',
            'cluster',
            'subCluster',
            'butirDirektorats.direktorat',
        ...
        $clusters = EksternalCluster::with('subClusters')->orderBy('nama_cluster')->get();
        ...
        return view('layouts.eksternal.tindak-lanjut', compact(
            'tindakLanjutRows',
    ...
    public function store(Request $request)
    {
        ...
        $butir = EksternalButir::with([
            'record.butirEksternal.butirPics',
            'record.butirEksternal.tindakLanjuts',
            'butirDirektorats',
            'butirPics.unitKerja',
            'tindakLanjuts',
        ])->findOrFail($validated['butir_id']);

        if (!$user || !$user->canCreateEksternalTindakLanjutForButir($butir)) {
        ...
        if (!in_array((int) $validated['unit_kerja_id'], $picUnitKerjaIds, true)) {
            return back()->with('error', 'PIC Unit yang dipilih tidak terdaftar pada butir EKSTERNAL ini.');
        }
        ...
        DB::connection('mysql_eksternal')->transaction(function () use ($request, $validated, $butir, $user) {
            $dokumenPath = null;

            if ($request->hasFile('dokumen')) {
                $dokumenPath = $request->file('dokumen')->store('dokumen/tindak-lanjut-eksternal', 'public');
            }

            $tindakLanjut = EksternalTindakLanjut::create([
            ...
            if ($record) {
                $record->loadMissing('butirEksternal.butirPics', 'butirEksternal.tindakLanjuts');

                $allButirsReady = $record->butirEksternal->count() > 0
                    && $record->butirEksternal->every(fn ($recordButir) => $recordButir->isTindakLanjutLengkap());

                $hasAnyTl = $record->butirEksternal->contains(fn ($recordButir) => $recordButir->tindakLanjuts->count() > 0);
            ...
            LogActivity::create([
                ...
                'action' => 'create',
                'description' => 'User membuat tindak lanjut EKSTERNAL.',
                'old_values' => null,
                'new_values' => [
        ...
        return redirect()
            ->route('eksternal.tindak-lanjut.index')
            ->with('success', 'Tindak lanjut EKSTERNAL berhasil disimpan.');
    }
}
