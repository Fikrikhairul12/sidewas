...
class ReviuDjsnController extends Controller
{
    public function index(Request $request)
    {
        $user = User::find(Auth::id());

        if (!$user || !$user->canAccessDjsnReview()) {
            abort(403, 'Anda tidak memiliki akses ke halaman reviu DJSN.');
        }
        ...
        return view('layouts.djsn.reviu', compact(
            'reviews',
            'clusters',
            'direktorats',
            'unitKerjas',
        ...
        // return view('layouts.djsn.reviu', compact('reviews'));
    }

    public function update(Request $request, DjsnReview $review)
    {
        $user = User::find(Auth::id());

        if (!$user || !$user->canReviewDjsnByKomite($review->komite_id)) {
        ...
        DB::connection('mysql_djsn')->transaction(function () use ($request, $validated, $review, $user) {
            ...
            if ($request->hasFile('dokumen')) {
                ...
                $dokumenPath = $request->file('dokumen')->store('dokumen/reviu-djsn', 'public');
            }
            ...
            LogActivity::create([
                ...
                'record_key' => $review->id_butir_djsn,
                ...
                'description' => $review->tahap_review === 'tindak_lanjut'
                    ? 'User melakukan reviu tindak lanjut DJSN.'
                    : 'User melakukan reviu tanggapan DJSN.',
                'old_values' => [
                    'review' => $oldReview,
        ...
        return redirect()
            ->route('djsn.reviu.index')
            ->with('success', 'Reviu DJSN berhasil disimpan.');
    }

    public function downloadDokumen(DjsnReview $review)
    {
        $user = User::find(Auth::id());
