    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm">
            ...
            <h1 class="mt-2 text-3xl font-bold text-slate-800">
                Cetak Report Keputusan RAGAB
            </h1>

            <p class="mt-2 text-sm text-slate-500">
                Pilih satu atau lebih surat Keputusan RAGAB untuk dicetak ke PDF atau Excel.
            </p>
        </div>
        ...
        <div class="rounded-2xl border border-blue-100 bg-white shadow-sm">
            <div class="border-b border-blue-50 px-6 py-5">
                <form method="GET" action="{{ route('ragab.report.index') }}">
                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="keyword" value="{{ request('keyword') }}"
                                placeholder="Cari nomor surat, perihal, agenda, keputusan, atau TL..."
                                class="w-full rounded-xl border-slate-300 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                            @forelse ($records as $record)
                                @php
                                    $butirsForReport = $record->butirRagab
                                        ->map(function ($butir) {
                                            return [
                                                'id' => $butir->id,
                                ...
                                <tr class="hover:bg-blue-50/40">
                                    ...
                                    <td class="px-6 py-6 align-top">
                                        <p class="text-sm font-bold text-slate-800">
                                            {{ $record->butir_ragab_count }} butir
                                        </p>

                                        @foreach ($record->butirRagab->take(2) as $butir)
                                            <p class="mt-2 text-xs text-slate-500">
                                                {{ $butir->tanggal_ragab ? \Carbon\Carbon::parse($butir->tanggal_ragab)->format('d/m/Y') : '-' }}
                                                —
                                                {{ \Illuminate\Support\Str::limit($butir->agenda_ragab ?? '-', 80) }}
                                            </p>
                                        @endforeach
                                    </td>
        ...
        <div id="reportFormatModal"
            ...
            <div class="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl">
                ...
                <div class="mt-6 grid gap-4 md:grid-cols-2">
                    <button type="submit" form="reportForm" formaction="{{ route('ragab.report.cetak') }}"
                    ...
                    <button type="submit" form="reportForm" formaction="{{ route('ragab.report.cetak-excel') }}"
                        formmethod="POST"
                        class="rounded-xl px-5 py-4 text-sm font-bold text-white shadow-sm hover:opacity-90"
                        style="background-color: #6bb17e;">
                        Cetak Excel
            ...
            <div class="w-full max-w-5xl rounded-2xl bg-white shadow-2xl">
                ...
                <form id="customReportForm" method="POST" class="px-6 py-6">
                    ...
                    <div class="mt-8 flex justify-end gap-3 border-t border-slate-100 pt-5">
                        ...
                        <button type="submit" formaction="{{ route('ragab.report.cetak-custom') }}"
                            formmethod="POST" formtarget="_blank"
                        ...
                        <button type="submit" formaction="{{ route('ragab.report.cetak-excel-custom') }}"
                            formmethod="POST"
                            class="rounded-xl px-5 py-3 text-sm font-bold text-white shadow-sm hover:opacity-90"
                            style="background-color: #6bb17e;">
                            Cetak Excel Custom
