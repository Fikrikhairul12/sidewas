    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm">
            ...
            <p class="mt-2 text-sm text-slate-500">
                Pilih satu atau lebih surat Keputusan RAWAS untuk dicetak ke PDF atau Excel.
            </p>
        </div>
        ...
        <div class="rounded-2xl border border-blue-100 bg-white shadow-sm">
            <div class="border-b border-blue-50 px-6 py-5">
                <form method="GET" action="{{ route('rawas.report.index') }}">
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white">
                            @forelse ($records as $record)
                                @php
                                    $butirsForReport = $record->butirRawas
                                        ->map(function ($butir) {
                                            return [
                                                'id' => $butir->id,
                                ...
                                <tr class="hover:bg-blue-50/40">
                                    <td class="px-6 py-6 text-center align-top">
                                        <input type="checkbox" name="record_ids[]" value="{{ $record->id }}"
                                            data-record-label="{{ $record->nomor_surat ?? $record->id_rawas }}"
                                    ...
                                    <td class="px-6 py-6 align-top">
                                        <p class="text-sm font-bold text-slate-800">
                                            {{ $record->butir_rawas_count }} butir
                                        </p>

                                        @foreach ($record->butirRawas->take(2) as $butir)
                                            <p class="mt-2 text-xs text-slate-500">
                                                {{ $butir->tanggal_rawas ? \Carbon\Carbon::parse($butir->tanggal_rawas)->format('d/m/Y') : '-' }}
                                                -
                                                {{ \Illuminate\Support\Str::limit($butir->agenda_rawas ?? '-', 80) }}
                                            </p>
                                        @endforeach
                                    </td>
        ...
        <div id="reportFormatModal"
            ...
            <div class="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl">
                ...
                <div class="mt-6 grid gap-4 md:grid-cols-2">
                    <button type="submit" form="reportForm" formaction="{{ route('rawas.report.cetak') }}"
                    ...
                    <button type="submit" form="reportForm" formaction="{{ route('rawas.report.cetak-excel') }}"
                        formmethod="POST"
                        class="rounded-xl px-5 py-4 text-sm font-bold text-white shadow-sm hover:opacity-90"
                        style="background-color: #6bb17e;">
                        Cetak Excel
            ...
            <div class="w-full max-w-5xl rounded-2xl bg-white shadow-2xl">
                ...
                <form id="customReportForm" method="POST" class="px-6 py-6">
                    ...
                    <div class="mt-8 flex justify-end gap-3 border-t border-slate-100 pt-5">
                        ...
                        <button type="submit" formaction="{{ route('rawas.report.cetak-custom') }}"
                            formmethod="POST" formtarget="_blank"
                        ...
                        <button type="submit" formaction="{{ route('rawas.report.cetak-excel-custom') }}"
                            formmethod="POST"
                            class="rounded-xl px-5 py-3 text-sm font-bold text-white shadow-sm hover:opacity-90"
                            style="background-color: #6bb17e;">
                            Cetak Excel Custom
