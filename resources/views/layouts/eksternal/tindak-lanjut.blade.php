    ...
    <div x-data="{
            ...
            $butirSiapTindakLanjut
                ->map(
                    fn($butir) => [
                        ...
                        'id_eksternal' => $butir->record?->id_eksternal,
                        'nomor_surat' => $butir->record?->nomor_surat,
                        'nama_instansi_pengundang' => $butir->record?->nama_instansi_pengundang,
                        'perihal_surat' => $butir->record?->perihal_surat,
                        'tanggal_eksternal_label' => $butir->tanggal_eksternal ? \Carbon\Carbon::parse($butir->tanggal_eksternal)->format('d/m/Y') : '-',
        ...
        get filteredButirs() {
            ...
            return this.butirs.filter((butir) => {
                return String(butir.id_butir_eksternal || '').toLowerCase().includes(keyword) ||
                    String(butir.id_eksternal || '').toLowerCase().includes(keyword) ||
                    String(butir.nomor_surat || '').toLowerCase().includes(keyword) ||
                    String(butir.nama_instansi_pengundang || '').toLowerCase().includes(keyword) ||
                    String(butir.perihal_surat || '').toLowerCase().includes(keyword) ||
                    String(butir.tanggal_eksternal_label || '').toLowerCase().includes(keyword) ||
                    String(butir.agenda_eksternal || '').toLowerCase().includes(keyword) ||
                    String(butir.keputusan_eksternal || '').toLowerCase().includes(keyword) ||
        ...
        selectButir(butir) {
            ...
            this.butirSearch = `${butir.id_butir_eksternal} - ${butir.nomor_surat ?? '-'}`;
        },
        ...
        <div x-show="openModal" x-transition.opacity
            ...
            <div @click.outside="openModal = false" x-transition
                ...
                <form method="POST" action="{{ route('eksternal.tindak-lanjut.store') }}" enctype="multipart/form-data"
                    class="px-6 py-6">
                    @csrf
                                            ...
                                            <button type="button" @click="selectButir(butir)"
                                                ...
                                                <div class="flex flex-col gap-1">
                                                    <p class="text-sm font-bold" style="color: #2377b9;"
                                                        x-text="butir.id_butir_eksternal"></p>

                                                    <p class="text-xs text-slate-500">
                                                        <span x-text="butir.nomor_surat ?? '-'"></span>
                                                        <span> • </span>
                                                        <span x-text="butir.tanggal_eksternal_label ?? '-'"></span>
                                                    </p>
                            ...
                            <template x-if="selectedButir">
                                <div class="mt-3 rounded-xl bg-blue-50 p-4">
                                    ...
                                    <p class="mt-2 text-sm text-slate-700">
                                        Tanggal & Agenda:
                                        <span class="font-bold" x-text="`${selectedButir.tanggal_eksternal_label} - ${selectedButir.agenda_eksternal ?? '-'}`"></span>
                                    </p>
